...
- **Snapshots** — `document.snapshot(artboard:stateMachine:size:fit:scale:at:)` renders a single frame headlessly to a `CGImage` (no view, window, or animation; closes the gap tracked by rive-ios #263/#265): pose is the state machine's initial state optionally advanced by a time offset, size defaults to the authored artboard size at 2x scale, rendering honors fit/alignment. Data binding is not applied — the frame shows the file's default values. Implemented through the legacy offscreen renderer under the same one-file isolation discipline as the bounds shim.
...
- **Schema generation** — `document.generateSchemaSource(named:artboard:viewModel:)` renders the discovered data-binding properties as ready-to-paste Swift source: a `RiveSchema` struct (one key per supported property, names sanitized and de-duplicated deterministically, nested paths preserved) plus a `RiveEnum` declaration per file enum, with unsupported properties listed in a trailing comment. Declarations are emitted `nonisolated` so the source compiles in modules with or without main-actor default isolation. The RiveInspector toolbar gets a "Copy Swift Schema" button.
...
- **Memory-pressure eviction** — `RiveEngine` automatically evicts completed cache entries on system memory pressure (warning/critical). Loaded views and instances keep working because they retain their document directly; only the next load of an evicted source re-parses. In-flight loads are never cancelled by pressure eviction. Opt out per engine with `setAutomaticEvictionEnabled(false)` (`isAutomaticEvictionEnabled` reads the current setting).
- **URLSession injection** — `RiveEngine.init(urlSession:)` threads a custom session through remote `.riv` downloads (including snapshot-time re-downloads); the shared engine keeps `URLSession.shared`.
- **Privacy manifest** — the library target ships an empty-collection `PrivacyInfo.xcprivacy` (no tracking, no collected data, no required-reason APIs).
- **Integration notes** — README section covering toolchain floors, coexistence with legacy `RiveRuntime` imports, schemas under `defaultIsolation(MainActor.self)`, the rive-ios version policy, app-extension caveats, and unsupported platforms. A weekly `upstream.yml` workflow builds and tests against the newest allowed rive-ios 6.x for early breakage warning.
